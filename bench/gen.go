...
func printTestCase(w io.Writer, filename string) {
	s := `
func BenchmarkDecode_{{.goodBaseName}}_webp(b *testing.B) {
...
func BenchmarkDecode_{{.goodBaseName}}_ximagewebp(b *testing.B) {
	data := tbLoadData(b, "{{.filename}}")
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkDecode_{{.goodBaseName}}_webp_tosize(b *testing.B) {
	data := tbLoadData(b, "{{.filename}}")
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
